...
void mostrarFraccion(FRACCION F1, FRACCION F2, FRACCION F3, FRACCION F4, FRACCION F5){
    ...
    mensajeImpropia(F1);
    mensajeImpropia(F2);
    printf("La fraccion mayor es %d/%d \n", obtenNum(F5), obtenDenom(F5));
}
void manejaMsg(int msg){
     char * mensajes[] = {"No hay memoria disponible . . .","Se ha liberado la memoria . . .  ", ""};
     printf("%s", mensajes[msg] );
}
void mensajeImpropia(FRACCION F){
     if(esImpropia(F)){
          printf("La fraccion %d/%d es impropia \n", obtenNum(F), obtenDenom(F));
     }else{
          printf("La fraccion %d/%d no es impropia \n", obtenNum(F), obtenDenom(F));
     }
}
